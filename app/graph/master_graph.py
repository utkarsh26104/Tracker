...
async def run_company(
    ...
    if seed_url:
        ...
        # fetch_company_site_findings), Amazon/Flipkart listings+reviews
        # (see search_marketplace_reviews), and Instagram/Facebook activity
        # (see search_social_media_presence), all run concurrently - gives
        # the Writer something real even if Tavily's normal news search
        # comes up empty. Marketplace/social sources matter especially for
        # a company whose own site is JS-rendered, where a plain HTML fetch
        # sees an empty shell and no real product/review content at all.
        site_findings, marketplace_findings, social_findings = await asyncio.gather(
            asyncio.to_thread(fetch_company_site_findings, seed_url),
            asyncio.to_thread(search_marketplace_reviews, company),
            asyncio.to_thread(search_social_media_presence, company),
        )
        seed_findings = site_findings + marketplace_findings + social_findings
        if seed_findings:
            ...
            if social_findings:
                source_parts.append(f"{len(social_findings)} social media post(s)")
